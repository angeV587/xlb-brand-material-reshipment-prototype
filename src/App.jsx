      ...
      <div className="pc-workspace">
        ...
        <section className="pc-main">
          ...
          <div className="pc-content-card">
            ...
            <div className="pc-table-wrap"><table className="pc-table"><thead><tr><th><input type="checkbox" aria-label="全选补发原因" checked={allReasonsSelected} onChange={toggleAllReasons} /></th><th>原因名称</th><th>原因说明</th><th>应用组织</th><th>绑定商品数</th><th>状态</th><th>修改人 / 修改时间</th><th>操作</th></tr></thead><tbody>{filteredReasons.map((item) => <tr key={item.code} className={selectedReasonCodes.includes(item.code) ? 'selected' : ''}><td><input type="checkbox" aria-label={`选择${item.name}`} checked={selectedReasonCodes.includes(item.code)} onChange={() => toggleReasonSelection(item.code)} /></td><td>{item.name}</td><td>{item.description}</td><td>{item.org}</td><td><button className="pc-link" type="button" onClick={() => openReasonDrawer('edit', item)}>{item.products} 个商品</button></td><td><span className={`pc-status ${item.status === '启用' ? 'on' : 'off'}`}>{item.status}</span></td><td>{item.editor}<small>{item.time}</small></td><td><button className="pc-link" type="button" onClick={() => openReasonDrawer('edit', item)}>编辑</button><button className="pc-link" type="button" onClick={() => toggleReason(item.code)}>{item.status === '启用' ? '停用' : '启用'}</button></td></tr>)}</tbody></table></div>
