...
{ id: 'BF202608150026', store: '星河路店', org: '华东事业部', reason: '物料破损补发', date: '2026-08-18', status: '审核', approvalStatus: '审批中', items: 2, qty: 3, applicant: '王小安', created: '2026-08-15 14:20', auditor: '—', auditedAt: '—', orderNo: '—', note: '会员活动立牌运输中破损', rejectReason: '', productLines: makeProductLines([['card', 2], ['stand', 1]]), productNames: '会员权益卡（新版） 会员活动立牌', productCodes: 'SP-202608-001 SP-202608-016' },
  ...
  { id: 'BF202608110003', store: '杭州湖滨店', org: '华东事业部', reason: '物料破损补发', date: '2026-08-14', status: '审核', approvalStatus: '已通过', items: 1, qty: 1, applicant: '陈晨', created: '2026-08-11 11:06', auditor: '李木子', auditedAt: '2026-08-11 13:18', orderNo: 'DD1001010000203', orderStatus: '已审核', note: '会员物料补发', rejectReason: '', productLines: makeProductLines([['stand', 1]]), productNames: '会员活动立牌', productCodes: 'SP-202608-016' },
...
const MOBILE_TABS = ['全部', '制单', '审核'];
const MOBILE_TIME_TYPES = ['制单时间', '审核时间'];
        ...
        {filtered.map((item) => (
          <article className="af-application-card" key={item.id} onClick={() => onOpen(item)}>
            ...
            <div className="af-card-body">
              ...
              <p><span>OA审批状态</span><b>{item.approvalStatus === '—' ? '—' : <StatusBadge status={item.approvalStatus} />}</b></p>
      ...
      {filterOpen && <div className="af-filter-mask" onClick={() => setFilterOpen(false)}>
        <section className="af-filter-sheet" onClick={(event) => event.stopPropagation()}>
          ...
          <div className="af-filter-content">
            ...
            <section><h3>OA审批状态</h3><div className="af-filter-options columns-3">{MOBILE_APPROVAL_STATUSES.map((item) => <button type="button" key={item} className={draftFilter.approvalStatus === item ? 'active' : ''} onClick={() => toggleApproval(item)}>{item}</button>)}</div></section>
          </div>
      ...
      <div className="af-order-form-scroll">
        ...
        <section className="af-form-document">
          ...
          <div className="af-form-document-fields">
            ...
            {initial && <label><span>OA审批状态</span><b>{initial.approvalStatus || '—'}</b></label>}
      ...
      <div className="af-detail-scroll">
        ...
        <section className="af-detail-document">
          ...
          <div className="af-detail-fields">
            ...
            <p><span>OA审批状态</span><b>{item.approvalStatus}</b></p>
            <p><span>制单人</span><b>{item.applicant}　{item.created}</b></p>
  ...
  return (
    <div className="paf-content">
      ...
      <section className="paf-filters">
        ...
        <label><span>单据状态</span><select value={status} onChange={(e) => setStatus(e.target.value)}><option>全部状态</option><option>制单</option><option>审核</option></select></label>
        <label><span>OA审批状态</span><select value={approvalStatus} onChange={(e) => setApprovalStatus(e.target.value)}><option>全部状态</option><option>审批中</option><option>已通过</option><option>已驳回</option></select></label>
      ...
      <div className="paf-table-wrap">
        <table>
          <thead><tr><th>申请单号</th><th>申请门店</th><th>所属组织</th><th>补发原因</th><th>商品/数量</th><th>单据状态</th><th>OA审批状态</th><th>关联门店订单</th><th>门店订单状态</th><th>申请人 / 申请时间</th><th>操作</th></tr></thead>
...
export function PcApplicationModule() {
  ...
  if (tab === 'list') {
    ...
    content = <PcApplicationForm initial={current} readonly={mode === 'view'} onBack={() => setTab('list')} onSave={(draft) => save(draft, '制单')} onSubmit={(draft) => save({ ...draft, approvalStatus: '审批中', rejectReason: '', auditor: '—', auditedAt: '—' }, '审核')} />;
  }
