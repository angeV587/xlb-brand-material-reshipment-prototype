...
{ id: 'BF202608150026', store: '星河路店', org: '华东事业部', reason: '物料破损补发', date: '2026-08-18', status: '审批中', items: 2, qty: 3, applicant: '王小安', created: '2026-08-15 14:20', auditor: '—', auditedAt: '—', voidedAt: '—', orderNo: '—', note: '会员活动立牌运输中破损', rejectReason: '', productLines: makeProductLines([['card', 2], ['stand', 1]]), productNames: '会员权益卡（新版） 会员活动立牌', productCodes: 'SP-202608-001 SP-202608-016' },
  ...
  { id: 'BF202608130011', store: '星河路店', org: '华东事业部', reason: '物料破损补发', date: '2026-08-16', status: '已审核', orderStatus: '已审核', items: 3, qty: 4, applicant: '王小安', created: '2026-08-13 16:42', auditor: '李木子', auditedAt: '2026-08-14 09:08', voidedAt: '—', orderNo: 'DD1001010000221', note: '会员活动物料破损补发', rejectReason: '', productLines: makeProductLines([['card', 1], ['stand', 1], ['poster', 2]]), productNames: '会员权益卡（新版） 会员活动立牌 会员活动海报', productCodes: 'SP-202608-001 SP-202608-016 SP-202608-021' },
  { id: 'BF202608120008', store: '苏州园区店', org: '华东事业部', reason: '版本换新补发', date: '2026-08-15', status: '已审核', orderStatus: '制单', items: 2, qty: 3, applicant: '周雨', created: '2026-08-12 09:31', auditor: '李木子', auditedAt: '2026-08-12 14:22', voidedAt: '—', orderNo: 'DD1001010000216', note: '商品属性校验导致门店订单审核失败，订单保留制单状态', rejectReason: '', productLines: makeProductLines([['card', 1], ['poster', 2]]), productNames: '会员权益卡（新版） 会员活动海报', productCodes: 'SP-202608-001 SP-202608-021' },
  { id: 'BF202608100007', store: '合肥政务区店', org: '华东事业部', reason: '物料破损补发', date: '2026-08-13', status: '已审核', items: 2, qty: 2, applicant: '陈晨', created: '2026-08-10 15:18', auditor: '李木子', auditedAt: '2026-08-11 09:26', voidedAt: '—', orderNo: '—', orderStatus: '—', note: '门店订单保存失败，待重试', orderError: '门店订单保存失败', rejectReason: '', productLines: makeProductLines([['card', 1], ['stand', 1]]), productNames: '会员权益卡（新版） 会员活动立牌', productCodes: 'SP-202608-001 SP-202608-016' },
...
function PcApplicationList({ applications, onCreate, onView, onOpenOrder, onRetry }) {
  ...
  return (
    <div className="paf-content">
      <header className="paf-title"><div><p>配送 / 业务操作</p><h1>品牌物料补发申请</h1></div><span><Workflow size={17} />所有申请统一提交泛微审批</span></header>
      <section className="paf-filters">
        ...
        <label><span>单据状态</span><select value={status} onChange={(e) => setStatus(e.target.value)}><option>全部状态</option><option>制单</option><option>审批中</option><option>已审核</option><option>已驳回</option><option>作废</option></select></label>
        <label><span>申请日期</span><div className="paf-range"><input type="date" /><i>至</i><input type="date" /></div></label>
        <footer><button type="button" className="primary"><Search size={15} />查询</button><button type="button" onClick={() => { setKeyword(''); setStatus('全部状态'); setReason('全部原因'); }}><RotateCcw size={15} />重置</button></footer>
      </section>
...
export function PcApplicationModule() {
  ...
  const openApplication = (item) => openForm(item, ['制单', '已驳回'].includes(item.status) ? 'edit' : 'view');
  ...
  if (tab === 'list') {
    ...
    content = <PcApplicationForm initial={current} readonly={mode === 'view'} onBack={() => setTab('list')} onSave={(draft) => save(draft, '制单')} onSubmit={(draft) => save(draft, '审批中')} />;
  }
