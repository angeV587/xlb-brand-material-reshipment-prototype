...
{ id: 'BF202608150026', store: '星河路店', org: '华东事业部', reason: '物料破损补发', date: '2026-08-18', status: '审核', approvalStatus: '审批中', items: 2, qty: 3, applicant: '王小安', created: '2026-08-15 14:20', auditor: '—', auditedAt: '—', orderNo: '—', note: '会员活动立牌运输中破损', rejectReason: '', productLines: makeProductLines([['card', 2], ['stand', 1]]), productNames: '会员权益卡（新版） 会员活动立牌', productCodes: 'SP-202608-001 SP-202608-016' },
  { id: 'BF202608140019', store: '南京中山路店', org: '华东事业部', reason: '新店开业补发', date: '2026-08-17', status: '制单', approvalStatus: '已驳回', items: 1, qty: 2, applicant: '李木子', created: '2026-08-14 10:12', auditor: '周雨', auditedAt: '2026-08-14 15:36', orderNo: '—', note: '新店开业首批会员物料缺失', rejectReason: '请补充商品破损图片后重新提交', productLines: makeProductLines([['poster', 2]]), productNames: '会员活动海报', productCodes: 'SP-202608-021' },
  ...
  { id: 'BF202608170001', store: '星河路店', org: '华东事业部', reason: '物料破损补发', date: '2026-08-19', status: '制单', approvalStatus: '—', items: 1, qty: 1, applicant: '王小安', created: '2026-08-17 08:46', auditor: '—', auditedAt: '—', orderNo: '—', note: '待补充商品图片', rejectReason: '', productLines: makeProductLines([['card', 1, false]]), productNames: '会员权益卡（新版）', productCodes: 'SP-202608-001' },
];
...
function MobileList({ applications, onCreate, onOpen }) {
  ...
  return (
    <main className="af-mobile-shell af-mobile-list">
      <header className="af-mobile-header"><span aria-hidden="true" /><h1>补发申请</h1><span aria-hidden="true" /></header>
...
function MobileForm({ initial, onBack, onSave, onSubmit }) {
  ...
  const validate = () => {
    if (!items.length) return '请至少选择一项补发商品';
    if (items.some((item) => !item.proof)) return '请为每项商品上传凭证图片';
  ...
  return (
    <main className="af-mobile-shell af-mobile-form af-order-form">
      <header className="af-mobile-header"><button type="button" onClick={onBack}><ArrowLeft size={22} /></button><h1>{initial ? '补发申请详情' : '新增补发申请'}</h1><button type="button" aria-label="更多操作" className="af-more-button">•••</button></header>
...
function MobileDetail({ item, onBack, onVoid }) {
  ...
  return (
    <main className="af-mobile-shell af-mobile-detail af-order-detail" data-layout-version="door-order-v2">
      <header className="af-mobile-header"><button type="button" onClick={onBack}><ArrowLeft size={22} /></button><h1>补发申请详情</h1><button type="button" aria-label="更多操作" className="af-more-button">•••</button></header>
    ...
    <main className="paf-app">
      ...
      <div className="paf-workspace">
        <aside>
          ...
          <section>
            <span>业务操作</span>
            {orderDetail ? <span>补发申请</span> : <b>补发申请</b>}
        ...
        <section className="paf-main">
          <nav className="paf-tabs">
            ...
            <button type="button" className={activeTab === 'list' ? 'active' : ''} onClick={() => onTab('list')}>补发申请</button>
...
function PcApplicationList({ applications, onCreate, onView, onOpenOrder, onRetry }) {
  ...
  return (
    <div className="paf-content">
      <header className="paf-title"><div><p>配送 / 业务操作</p><h1>补发申请</h1></div></header>
  ...
  return (
    <div className="paf-content paf-store-order-detail">
      ...
      <section className="paf-order-head-grid">
        ...
        <p><span>业务来源</span><b>补发申请</b></p>
        <p><span>来源申请单</span><b>{application.id}</b></p>
        <p className="wide"><span>留言备注</span><b>【由补发申请生成的门店订单】</b></p>
      </section>
